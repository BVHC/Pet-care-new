 ...
 * cho {@code OrderCreated}/{@code OrderCancelled}/{@code OrderTimedOut} — chỉ 3 event có call site
 * trong phạm vi task này ({@code CheckoutOrder} chỉ refresh TTL, không đổi state, không có event
 * riêng theo đúng FSM table). Không có {@code @Transactional} riêng — join transaction đang mở
 * của caller.
 */
@Component
