...
- Lock / Unlock / Deactivate thuộc IAM, không đưa vào Auth API.
...
| Login | Customer / Staff | Auth (01) | Credentials đúng, Account `ACTIVE` | RULE-01-01 (chỉ ACTIVE + phát hành JWT/Session Token), RULE-01-07 (5 sai liên tiếp → LOCKED ≥15p), RULE-02-04 (LOCKED từ chối mọi op) | POST | `/auth/login` | Public | Self (đúng credentials) | Thành công: không đổi state. Thất bại lần 5 liên tiếp: `ACTIVE → LOCKED`, ev `AccountLocked` | — | — | Non-idempotent (mỗi success đẻ session mới; mỗi fail đều đếm) | Identifier phone-hoặc-email ASSUMPTION A1; Bearer transport ASSUMPTION A2; TTL token TBD Q4 |
...
(chữ "liên tiếp" ở RULE-01-07) · A5 hết `locked_until` thì được login lại
(chữ "tối thiểu 15 phút" + cột `locked_until`) · A6 resend cho account đã ACTIVE
→ từ chối (hiển nhiên từ FSM).
...
- **Status:** `201` created · `400` validation · `409` phone/email đã tồn tại
  (ERD UK CONFIRMED) · `429` resend-limit nếu lạm dụng · `422` sai định dạng nghiệp vụ.
- **Error codes (PROPOSED, trừ khi ghi chú):** `VALIDATION_FAILED`,
  `PHONE_ALREADY_EXISTS`, `EMAIL_ALREADY_EXISTS`, `OTP_SEND_FAILED`
...
- **Validation:** OTP đúng phone, chưa `is_used` (ERD CONFIRMED),
  `now ≤ expires_at` (RULE-01-02/08 CONFIRMED), `attempt_count < 5` (RULE-01-05 CONFIRMED).
...
- **Response 200:** `{accessToken, refreshToken (cặp này CONFIRMED tồn tại theo
  ...
  user: {userId, accountId, role (9 enum CONFIRMED), mustChangePassword
...
- **Status:** `200` · `400` · `401` credentials sai (`INVALID_CREDENTIALS` PROPOSED,
  ...
  CONFIRMED) · `429` brute-force (rate limit là derived, ngưỡng TBD Q11).
...
- **State transition:** success → none (counter reset A4); fail thứ 5 liên tiếp →
  `ACTIVE → LOCKED` + `AccountLocked` (RULE-01-07 CONFIRMED); auto-unlock sau
  `locked_until` là A5.
...
| Q2 | Response envelope + error shape (đề xuất `{error:{code,message}}`, code string) | TBD (PO) | docs chỉ cho 1 mã `ACCESS_DENIED_SCOPE_MISMATCH` |
...
| Q4 | Access/refresh TTL + rotation policy + reuse-handling | TBD (PO) | docs không có |
...
Mọi TBD (Q1–Q11) được giữ trong `description`, không bịa giá trị (không `servers`,
không `expiresIn`, không password constraints).
