...
- `inventory_reservations` (giữ 15m cho đơn Online) là effect nội bộ của M14 —
  không endpoint.
- Theo dõi lô/hạn hàng tổng hợp (`TrackBatch`/`TrackExpiry`): ERD không có bảng lô
  chung (chỉ `vaccine_batches` ở M10) → read `GET …/inventory-batches` shape
  PROPOSED + storage TBD Q9, không phát minh bảng.
...
| 5 | `GET /stores/{id}/inventory-batches` | `TrackBatch`/`TrackExpiry` (read) — `01#12`, RULE-12-11 |
...
| ReceiveInventory | InventoryStaff | — | — | POST | `…/inventory/receive` | Bearer | Staff Store đó | + physical/available | Non-idempotent (mỗi call 1 lần nhập) | `{productId, quantity, batchNumber?, expiryDate?}` PROPOSED (batch storage TBD Q9) |
...
| TrackBatch/Expiry | InventoryStaff | — | RULE-12-11 (FEFO) | GET | `…/inventory-batches` | Bearer | Staff scope | none | Idempotent | Shape PROPOSED, storage TBD Q9 |
...
- **`POST …/inventory/receive`** — `{productId (req), quantity (req, >0),
  batchNumber?, expiryDate?}`. Response `200 {…số mới}`.
- **`POST …/inventory/issue`** — `{productId (req), quantity (req, >0), reason?}`.
  Guards RULE-12-05: `quantity ≤ available`, lô không hết hạn (khi có batch).
  Vi phạm → `409`.
- **`POST …/inventory/count`** — `{productId (req), countedQuantity (req, ≥0)}`.
  Chênh → sinh adjustment `COUNT_VARIANCE` PENDING (A2, response kèm
  `adjustmentId?`); khớp → chỉ log (response `variance: 0`).
...
| Q5 | Count khớp (variance 0) có lưu biên bản kiểm kê không (A2: chỉ log)? | TBD (PO) | RULE-12-02 |
...
| Q9 | Lô/hạn hàng tổng hợp lưu ở đâu (TrackBatch/Expiry thiếu bảng)? | TBD (BE) | ERD thiếu |
