...
| BC-009 | **Pet Schedule Collision Guard áp dụng toàn hệ thống chi nhánh:** Một Pet không được có hai lịch phục vụ hoạt động chồng giờ dù ở cùng Store hay khác Store trong cùng Organization (và về nguyên tắc là toàn nền tảng, do kiểm tra không giới hạn theo Store — xem OPEN-APT-01 §10 để làm rõ phạm vi chính xác). |
...
| ID | Mô tả |
|---|---|
| **AMBIGUITY-01 (Grace Period No-Show)** | `RULE-06-09` mô tả thời gian ân hạn trước khi đánh dấu vắng mặt là *"quy định, ví dụ: 15 phút"* — dùng chữ "ví dụ", ngụ ý đây là giá trị minh họa/cấu hình được, không phải hằng số bắt buộc như Hold TTL (900s chính xác) hay OTP TTL (300s chính xác). Nhưng bảng FSM (`03` §4.2) lại liệt kê thẳng *"Grace Period 15 phút"* như một giá trị cố định. Chưa rõ: Grace Period là hằng số hệ thống hay tham số cấu hình theo Store/Organization? |
| **AMBIGUITY-02 (Loyalty Point Expiry)** | `RULE-19-07`: *"Điểm tích lũy có thời hạn hiệu lực theo chính sách (ví dụ: 12 tháng...)"* — không xác định giá trị chính thức, chỉ nêu ví dụ. Cần quyết định: giá trị mặc định toàn nền tảng, hay bắt buộc mỗi Organization tự cấu hình? |
| **AMBIGUITY-03 (Low-Stock / Expiry Warning Threshold)** | `RULE-12-12` nêu ngưỡng cảnh báo tồn kho thấp và cảnh báo hết hạn chỉ bằng ví dụ ("30/60/90 ngày trước hạn"), không có giá trị mặc định chính thức hay quy định ai được quyền cấu hình (Store hay Organization). |
| **AMBIGUITY-04 (Promotion + Voucher Stacking)** | `RULE-18-08` chỉ quy định rõ ràng việc không cộng dồn giữa **nhiều Voucher** với nhau ("tối đa 01 Voucher... trừ khi... cho phép cộng dồn"). Không có quy tắc tường minh cho việc một **Promotion** (khuyến mãi tự động theo điều kiện) có được áp dụng đồng thời với một **Voucher** (mã nhập tay) trên cùng một hóa đơn/đơn hàng hay không. |
| **AMBIGUITY-05 (Phạm vi Pet Schedule Collision Guard)** | `RULE-06-11` nói rõ kiểm tra xung đột lịch Pet áp dụng *"trên toàn bộ hệ thống chi nhánh"*, và Technical Invariant liên quan ghi chú "Platform / Store / Pet / Appointment / Collision" ở cột Scope. Điều này ngụ ý kiểm tra xung đột xuyên suốt **toàn nền tảng** (mọi Organization), điều này mâu thuẫn tiềm ẩn với nguyên tắc cách ly dữ liệu 100% giữa các Organization (`RULE-02-01`, `RULE-03-01`) — vì một Pet vốn chỉ thuộc về một Customer trong phạm vi dữ liệu của đúng một Organization sở hữu Store nơi Pet được đăng ký, nên về logic một Pet không thể "đặt lịch" ở Organization khác nếu dữ liệu Pet không được chia sẻ liên Organization. Cần làm rõ: "toàn hệ thống chi nhánh" nghĩa là toàn bộ Store **trong cùng một Organization**, hay literal toàn nền tảng? |
