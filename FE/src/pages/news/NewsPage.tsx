  ...
  return (
    <div className="bg-[var(--color-surface-page)] pb-24">
      ...
      <section className="mx-auto max-w-[1200px] px-5 pt-12 sm:px-8">
            ...
            <button
              ...
              className={`rounded-full px-5 py-2 text-[13.5px] font-semibold transition-all duration-200 ${
                selectedCategory === cat
                  ...
                  : 'bg-[var(--color-surface-card)] text-[var(--color-text-primary)] hover:bg-[var(--color-accent-soft)] hover:text-[var(--color-accent)]'
              }`}
            >
              {cat}
            </button>
        ...
        {/* Featured */}
        <article className="mb-10 grid overflow-hidden rounded-3xl border border-[var(--color-border-default)] bg-[var(--color-surface-card)] shadow-[0_1px_2px_rgba(56,36,23,0.06),0_8px_18px_-14px_rgba(56,36,23,0.4)] lg:grid-cols-2">
          <div className="aspect-[4/3] bg-[var(--color-surface-1)] lg:aspect-auto">
            <img src={featured.image} alt="" className="h-full w-full object-cover" />
          </div>
          <div className="flex flex-col justify-center p-8">
            <span className="inline-block w-fit rounded-full bg-[var(--color-accent-soft)] px-3 py-1 text-[11.5px] font-bold uppercase tracking-wide text-[var(--color-accent)]">
              {featured.category} · Nổi bật
            </span>
            <h2 className="mt-4 font-[var(--font-friendly)] text-[26px] font-extrabold leading-tight text-[var(--color-text-primary)]">
              {featured.title}
            </h2>
            <p className="mt-3 text-[14px] leading-relaxed text-[var(--color-text-secondary)]">
              {featured.excerpt}
            </p>
            <div className="mt-4 flex items-center gap-4 text-[12.5px] text-[var(--color-text-secondary)]">
            ...
            <button className="mt-6 inline-flex w-fit items-center gap-2 text-[13.5px] font-bold text-[var(--color-accent)] transition-colors hover:text-[var(--color-accent-hover)]">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {rest.map((article) => (
            <article
              key={article.id}
              className="group overflow-hidden rounded-2xl border border-[var(--color-border-default)] bg-[var(--color-surface-card)] shadow-[0_1px_2px_rgba(56,36,23,0.06),0_4px_12px_-8px_rgba(56,36,23,0.4)] transition-all duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] hover:-translate-y-0.5 hover:shadow-[0_2px_4px_rgba(56,36,23,0.07),0_12px_28px_-16px_rgba(164,51,36,0.4)]"
            >
              <div className="aspect-[4/3] overflow-hidden bg-[var(--color-surface-1)]">
                <img
                  src={article.image}
                  alt=""
              ...
              <div className="p-5">
                <span className="inline-block rounded-full bg-[var(--color-accent-soft)] px-2.5 py-0.5 text-[11px] font-bold text-[var(--color-accent)]">
                  {article.category}
                </span>
                <h3 className="mt-3 line-clamp-2 text-[15px] font-bold text-[var(--color-text-primary)] transition-colors group-hover:text-[var(--color-accent)]">
                  {article.title}
                </h3>
                <p className="mt-2 line-clamp-2 text-[13px] text-[var(--color-text-secondary)]">
                  {article.excerpt}
                </p>
                <div className="mt-4 flex items-center justify-between text-[12px] text-[var(--color-text-secondary)]">
                  <span className="flex items-center gap-1">
        ...
        {rest.length === 0 && (
          <div className="rounded-2xl border border-dashed border-[var(--color-border-default)] bg-[var(--color-surface-card)] p-12 text-center text-[var(--color-text-secondary)]">
