...
export function SiteFooter({ onNav }: SiteFooterProps) {
  ...
  return (
    <footer className="border-t border-[var(--color-border-default)] bg-[var(--color-surface-1)] px-6 pt-14 pb-7">
      {/* Newsletter */}
      <div className="mx-auto mb-10 max-w-6xl rounded-2xl bg-gradient-to-br from-[var(--color-accent)] to-[var(--color-accent-hover)] p-6 sm:p-8">
        <div className="grid items-center gap-6 sm:grid-cols-2">
          <div className="text-white">
            <h3 className="font-[var(--font-friendly)] text-xl font-extrabold leading-tight">
          ...
          >
            <div className="relative flex-1">
              ...
              <input
                ...
                className="h-11 w-full rounded-full border-none bg-white pl-9 pr-3 text-[13.5px] text-[var(--color-text-primary)] outline-none focus:ring-2 focus:ring-white/40"
              />
            </div>
            <button
              type="submit"
      ...
      <div className="mx-auto grid max-w-6xl grid-cols-2 gap-8 pb-8 sm:grid-cols-3 lg:grid-cols-[1.4fr_1fr_1fr_1fr_1.1fr]">
        <div className="col-span-2 sm:col-span-3 lg:col-span-1">
          <div
            className="mb-3 cursor-pointer font-[var(--font-friendly)] text-xl font-extrabold text-[var(--color-text-primary)] flex items-center gap-2"
            onClick={() => onNav('home')}
          >
            <img src="/imgs/DogSticker.svg" alt="PetCare Logo" className="h-6 w-auto object-contain" /> PetCare
          </div>
          <p className="max-w-[260px] text-[13px] leading-relaxed text-[var(--color-text-secondary)]">
          ...
          <div className="mt-4 flex items-center gap-2">
            {[
              ...
              <a
                ...
                aria-label={s.label}
                className="flex h-9 w-9 items-center justify-center rounded-full border border-[var(--color-border-default)] bg-[var(--color-surface-card)] text-[var(--color-text-secondary)] transition-all hover:border-[var(--color-accent)] hover:bg-[var(--color-accent-soft)] hover:text-[var(--color-accent)]"
              >
                <s.Icon size={15} />
              </a>
            ))}
      ...
      {/* Contact strip */}
      <div className="mx-auto mb-6 grid max-w-6xl gap-3 border-y border-[var(--color-border-default)] py-5 text-[12.5px] text-[var(--color-text-secondary)] sm:grid-cols-3">
        <div className="flex items-center gap-2">
          <Phone size={14} className="shrink-0 text-[var(--color-accent)]" />
          <span>
            Hotline: <strong className="text-[var(--color-text-primary)]">1900 1234</strong> · 8:00–20:00
          </span>
        </div>
        <div className="flex items-center gap-2">
          <Mail size={14} className="shrink-0 text-[var(--color-accent)]" />
      ...
      {/* Bottom */}
      <div className="mx-auto flex max-w-6xl flex-wrap items-center gap-x-5 gap-y-2 text-[13px] text-[var(--color-text-secondary)]">
...
function FooterCol({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div>
      ...
      <div className="flex flex-col gap-2.5 [&_a]:text-sm [&_a]:text-[var(--color-text-primary)] [&_a]:no-underline [&_a:hover]:text-[var(--color-accent)] [&_a]:transition-colors">
        {children}
      </div>
    </div>
  );
